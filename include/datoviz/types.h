...
struct DvzVolumeState
{
    ...
    double iso_threshold;
    DvzIsosurfaceMode iso_mode;
    float iso_color[4];        /* flat base color used when no transfer/colormap texture is set */
    bool iso_use_value_color;  /* if true, sample color from the transfer texture at the hit value */
    float iso_gradient_step;   /* central-difference step, in normalized UVW texels; 0 = auto */
    float iso_light_dir[3];    /* key light direction, view space, normalized */
    float iso_ambient;
    float iso_diffuse;
    float iso_specular;
    float iso_shininess;
